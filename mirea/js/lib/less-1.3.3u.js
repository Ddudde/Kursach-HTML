(function(window, undefined) {
  ...
  function LessError(e, env) {
    var input = getInput(e, env), loc = getLocation(e.index, input), line = loc.line, col = loc.column, lines = input.split('\n');
  ...
  frames: this.frames.concat(env.frames)}, args, [])].concat(env.frames)})) {
    return false;
  }
  return true;
}, 
  ...
  var css = [], rules = [], _rules = [], rulesets = [], paths = [], selector, debugInfo, rule;
  if (!this.root) {
      ...
      for (j = 0; j < newSelectors.length; j++) {
        ...
        if (context.length == 0) {
          if (sel.length > 0) {
            ...
            sel[0].elements.push(new (tree.Element)(el.combinator, '', 0));
          }
  ...
  dumpLineNumbers: less.dumpLineNumbers}).parse(styles[i].innerHTML || '', function(e, tree) {
  var css = tree.toCSS();
  var style = styles[i];
  style.type = 'text/css';
  if (style.styleSheet) {
  ...
  function extractUrlParts(url, baseUrl) {
    var urlPartsRegex = /^((?:[a-z-]+:)?\/\/(?:[^\/\?#]*\/)|([\/\\]))?((?:[^\/\\\?#]*[\/\\])*)([^\/\\\?#]*)([#\?].*)?$/, urlParts = url.match(urlPartsRegex), returner = {}, directories = [], i, baseUrlParts;
    if (!urlParts) {
  ...
  contents: contents, 
  files: files, 
  dumpLineNumbers: less.dumpLineNumbers}).parse(data, function(e, root) {
  if (e) {
    return error(e, href);
  }
  try {
